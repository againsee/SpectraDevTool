(function(b){function a(){var c=this;var i;var e=false;var h=false;var d={INPUT_STATUS:{id:"INPUT_STATUS",callback:null},SUPPORT_STATUS:{id:"SUPPORT_STATUS",callback:null},NEW_MESSAGE:{id:"NEW_MESSAGE",callback:null},READ_MESSAGE:{id:"READ_MESSAGE",callback:null},TICKET_ROUTED:{id:"TICKET_ROUTED",callback:null},TICKET_ENDED:{id:"TICKET_ENDED",callback:null},TICKET_EXPIRED:{id:"TICKET_EXPIRED",callback:null},FORWARD_ACCEPT:{id:"FORWARD_ACCEPT",callback:null},FORWARD_COMPLETE:{id:"FORWARD_COMPLETE",callback:null},FORWARD_RETURN:{id:"FORWARD_RETURN",callback:null},ROUTING_RETURN:{id:"ROUTING_RETURN",callback:null},TICKET_DELETED:{id:"TICKET_DELETED",callback:null},ROUTING_FLAG_CHANGED:{id:"ROUTING_FLAG_CHANGED",callback:null},CONNECTION_ESTABLISHED:{id:"CONNECTION_ESTABLISHED",callback:null},CONNECTION_BROKEN:{id:"CONNECTION_BROKEN",callback:null}};this.EVENT=d;this.addListener=function(j,k){j.callback=k};this.receive=function(m){var l=m.data.__event_id__;var k=m.data.talkId;var o=m.data.serviceType;var j=m.data.updatedBy;var n=m.data.value;if(!d[l].callback){return}switch(l){case d.INPUT_STATUS.id:d[l].callback(k,o,m.data.status,m.data.userType);break;case d.SUPPORT_STATUS.id:d[l].callback(k,o,m.data.status);break;case d.NEW_MESSAGE.id:d[l].callback(k,o,m.data.userType);break;case d.READ_MESSAGE.id:d[l].callback(k,m.data.seq,o);break;case d.TICKET_ROUTED.id:d[l].callback(k,o);break;case d.TICKET_ENDED.id:d[l].callback(k,o);break;case d.TICKET_EXPIRED.id:d[l].callback(k,o);break;case d.FORWARD_ACCEPT.id:d[l].callback(k,o);break;case d.FORWARD_COMPLETE.id:d[l].callback(k,o);break;case d.FORWARD_RETURN.id:d[l].callback(k,o);break;case d.ROUTING_RETURN.id:d[l].callback(k,o,j);break;case d.TICKET_DELETED.id:d[l].callback(k,o,j);break;case d.ROUTING_FLAG_CHANGED.id:d[l].callback(o,n);break}};b.cometd.addListener("/meta/handshake",function(j){b.cometd.batch(function(){b.cometd.subscribe("/talk/event",c.receive);b.cometd.publish("/service/members",{user:i,status:"associate"})})});b.cometd.addListener("/meta/connect",function(j){e=h;h=j.successful===true;if(!e&&h){f()}else{if(e&&!h){g()}}});function f(){if(d.CONNECTION_ESTABLISHED.callback){d.CONNECTION_ESTABLISHED.callback()}}function g(){if(d.CONNECTION_BROKEN.callback){d.CONNECTION_BROKEN.callback()}}this.init=function(l,m){var j=this;i=m;var k=location.protocol+"//"+l.host+":"+l.port+"/gateway/cometd";b.cometd.init({url:k,logLevel:"info"});b(window).unload(function(){b.cometd.reload({cookieMaxAge:10});j.disconnect()})};this.initCustomer=function(j,k){this.init(j,"C_"+k)};this.initAgent=function(j,k){this.init(j,"A_"+k)};this.disconnect=function(){b.cometd.publish("/service/members",{user:i,status:"disassociate"})}}b.gateway=new a()})(jQuery);
/*!
...
(function(a){a.cometd.registerExtension("ack",new org.cometd.AckExtension())})(jQuery);
(function(a){if(a.cookie){org.cometd.COOKIE.set=a.cookie;org.cometd.COOKIE.get=a.cookie}a.cometd.registerExtension("reload",new org.cometd.ReloadExtension())})(jQuery);
(function(a){a.cometd.registerExtension("timestamp",new org.cometd.TimeStampExtension())})(jQuery);
(function(a){a.cometd.registerExtension("timesync",new org.cometd.TimeSyncExtension())})(jQuery);
(function(b){org.cometd.JSON.toJSON=JSON.stringify;org.cometd.JSON.fromJSON=JSON.parse;function a(f,e){if(e){for(var g in e){if(g.toLowerCase()==="content-type"){continue}f.setRequestHeader(g,e[g])}}}function d(){var f=new org.cometd.LongPollingTransport();var e=org.cometd.Transport.derive(f);e.xhrSend=function(g){return b.ajax({url:g.url,async:g.sync!==true,type:"POST",contentType:"application/json;charset=UTF-8",data:g.body,xhrFields:{withCredentials:true},beforeSend:function(h){a(h,g.headers);return true},success:g.onSuccess,error:function(j,i,h){g.onError(i,h)}})};return e}function c(){var f=new org.cometd.CallbackPollingTransport();var e=org.cometd.Transport.derive(f);e.jsonpSend=function(g){b.ajax({url:g.url,async:g.sync!==true,type:"GET",dataType:"jsonp",jsonp:"jsonp",data:{message:g.body},beforeSend:function(h){a(h,g.headers);return true},success:g.onSuccess,error:function(j,i,h){g.onError(i,h)}})};return e}b.Cometd=function(f){var e=new org.cometd.Cometd(f);if(org.cometd.WebSocket){e.registerTransport("websocket",new org.cometd.WebSocketTransport())}e.registerTransport("long-polling",new d());e.registerTransport("callback-polling",new c());return e};b.cometd=new b.Cometd()})(jQuery);
/*!
...
if(typeof dojo!=="undefined"){dojo.provide("org.cometd")}else{this.org=this.org||{};org.cometd={}}org.cometd.JSON={};org.cometd.JSON.toJSON=org.cometd.JSON.fromJSON=function(a){throw"Abstract"};org.cometd.Utils={};org.cometd.Utils.isString=function(a){if(a===undefined||a===null){return false}return typeof a==="string"||a instanceof String};org.cometd.Utils.isArray=function(a){if(a===undefined||a===null){return false}return a instanceof Array};org.cometd.Utils.inArray=function(b,c){for(var a=0;a<c.length;++a){if(b===c[a]){return a}}return -1};org.cometd.Utils.setTimeout=function(a,c,b){return window.setTimeout(function(){try{c()}catch(d){a._debug("Exception invoking timed function",c,d)}},b)};org.cometd.Utils.clearTimeout=function(a){window.clearTimeout(a)};org.cometd.TransportRegistry=function(){var b=[];var a={};this.getTransportTypes=function(){return b.slice(0)};this.findTransportTypes=function(d,h,e){var c=[];for(var f=0;f<b.length;++f){var g=b[f];if(a[g].accept(d,h,e)===true){c.push(g)}}return c};this.negotiateTransport=function(g,c,k,e){for(var f=0;f<b.length;++f){var h=b[f];for(var d=0;d<g.length;++d){if(h===g[d]){var l=a[h];if(l.accept(c,k,e)===true){return l}}}}return null};this.add=function(e,g,c){var f=false;for(var d=0;d<b.length;++d){if(b[d]===e){f=true;break}}if(!f){if(typeof c!=="number"){b.push(e)}else{b.splice(c,0,e)}a[e]=g}return !f};this.find=function(d){for(var c=0;c<b.length;++c){if(b[c]===d){return a[d]}}return null};this.remove=function(d){for(var c=0;c<b.length;++c){if(b[c]===d){b.splice(c,1);var e=a[d];delete a[d];return e}}return null};this.clear=function(){b=[];a={}};this.reset=function(){for(var c=0;c<b.length;++c){a[b[c]].reset()}}};org.cometd.Transport=function(){var a;var b;this.registered=function(d,c){a=d;b=c};this.unregistered=function(){a=null;b=null};this._debug=function(){b._debug.apply(b,arguments)};this._mixin=function(){return b._mixin.apply(b,arguments)};this.getConfiguration=function(){return b.getConfiguration()};this.getAdvice=function(){return b.getAdvice()};this.setTimeout=function(d,c){return org.cometd.Utils.setTimeout(b,d,c)};this.clearTimeout=function(c){org.cometd.Utils.clearTimeout(c)};this.convertToMessages=function(d){if(org.cometd.Utils.isString(d)){try{return org.cometd.JSON.fromJSON(d)}catch(c){this._debug("Could not convert to JSON the following string",'"'+d+'"');throw c}}if(org.cometd.Utils.isArray(d)){return d}if(d===undefined||d===null){return[]}if(d instanceof Object){return[d]}throw"Conversion Error "+d+", typeof "+(typeof d)};this.accept=function(c,e,d){throw"Abstract"};this.getType=function(){return a};this.send=function(c,d){throw"Abstract"};this.reset=function(){this._debug("Transport",a,"reset")};this.abort=function(){this._debug("Transport",a,"aborted")};this.toString=function(){return this.getType()}};org.cometd.Transport.derive=function(a){function b(){}b.prototype=a;return new b()};org.cometd.RequestTransport=function(){var k=new org.cometd.Transport();var h=org.cometd.Transport.derive(k);var g=0;var a=null;var d=[];var i=[];function c(p){while(i.length>0){var o=i[0];var n=o[0];var m=o[1];if(n.url===p.url&&n.sync===p.sync){i.shift();p.messages=p.messages.concat(n.messages);this._debug("Coalesced",n.messages.length,"messages from request",m.id);continue}break}}function l(q,p){this.transportSend(q,p);p.expired=false;if(!q.sync){var o=this.getConfiguration().maxNetworkDelay;var n=o;if(p.metaConnect===true){n+=this.getAdvice().timeout}this._debug("Transport",this.getType(),"waiting at most",n,"ms for the response, maxNetworkDelay",o);var m=this;p.timeout=this.setTimeout(function(){p.expired=true;if(p.xhr){p.xhr.abort()}var r="Request "+p.id+" of transport "+m.getType()+" exceeded "+n+" ms max network delay";m._debug(r);m.complete(p,false,p.metaConnect);q.onFailure(p.xhr,q.messages,"timeout",r)},n)}}function f(o){var n=++g;var m={id:n,metaConnect:false};if(d.length<this.getConfiguration().maxConnections-1){d.push(m);l.call(this,o,m)}else{this._debug("Transport",this.getType(),"queueing request",n,"envelope",o);i.push([o,m])}}function j(m){var n=m.id;this._debug("Transport",this.getType(),"metaConnect complete, request",n);if(a!==null&&a.id!==n){throw"Longpoll request mismatch, completing request "+n}a=null}function e(q,s){var p=org.cometd.Utils.inArray(q,d);if(p>=0){d.splice(p,1)}if(i.length>0){var o=i.shift();var n=o[0];var r=o[1];this._debug("Transport dequeued request",r.id);if(s){if(this.getConfiguration().autoBatch){c.call(this,n)}f.call(this,n);this._debug("Transport completed request",q.id,n)}else{var m=this;this.setTimeout(function(){m.complete(r,false,r.metaConnect);n.onFailure(r.xhr,n.messages,"error","Previous request failed")},0)}}}h.complete=function(m,o,n){if(n){j.call(this,m)}else{e.call(this,m,o)}};h.transportSend=function(n,m){throw"Abstract"};h.transportSuccess=function(o,m,n){if(!m.expired){this.clearTimeout(m.timeout);this.complete(m,true,m.metaConnect);if(n&&n.length>0){o.onSuccess(n)}else{o.onFailure(m.xhr,o.messages,"Empty HTTP response")}}};h.transportFailure=function(p,n,o,m){if(!n.expired){this.clearTimeout(n.timeout);this.complete(n,false,n.metaConnect);p.onFailure(n.xhr,p.messages,o,m)}};function b(o){if(a!==null){throw"Concurrent metaConnect requests not allowed, request id="+a.id+" not yet completed"}var n=++g;this._debug("Transport",this.getType(),"metaConnect send, request",n,"envelope",o);var m={id:n,metaConnect:true};l.call(this,o,m);a=m}h.send=function(m,n){if(n){b.call(this,m)}else{f.call(this,m)}};h.abort=function(){k.abort();for(var m=0;m<d.length;++m){var n=d[m];this._debug("Aborting request",n);if(n.xhr){n.xhr.abort()}}if(a){this._debug("Aborting metaConnect request",a);if(a.xhr){a.xhr.abort()}}this.reset()};h.reset=function(){k.reset();a=null;d=[];i=[]};return h};org.cometd.LongPollingTransport=function(){var b=new org.cometd.RequestTransport();var a=org.cometd.Transport.derive(b);var c=true;a.accept=function(d,f,e){return c||!f};a.xhrSend=function(d){throw"Abstract"};a.transportSend=function(h,g){this._debug("Transport",this.getType(),"sending request",g.id,"envelope",h);var f=this;try{var e=true;g.xhr=this.xhrSend({transport:this,url:h.url,sync:h.sync,headers:this.getConfiguration().requestHeaders,body:org.cometd.JSON.toJSON(h.messages),onSuccess:function(j){f._debug("Transport",f.getType(),"received response",j);var k=false;try{var l=f.convertToMessages(j);if(l.length===0){c=false;f.transportFailure(h,g,"no response",null)}else{k=true;f.transportSuccess(h,g,l)}}catch(i){f._debug(i);if(!k){c=false;f.transportFailure(h,g,"bad response",i)}}},onError:function(j,i){c=false;if(e){f.setTimeout(function(){f.transportFailure(h,g,j,i)},0)}else{f.transportFailure(h,g,j,i)}}});e=false}catch(d){c=false;this.setTimeout(function(){f.transportFailure(h,g,"error",d)},0)}};a.reset=function(){b.reset();c=true};return a};org.cometd.CallbackPollingTransport=function(){var c=new org.cometd.RequestTransport();var a=org.cometd.Transport.derive(c);var b=2000;a.accept=function(d,f,e){return true};a.jsonpSend=function(d){throw"Abstract"};a.transportSend=function(n,k){var r=this;var e=0;var f=n.messages.length;var h=[];while(f>0){var q=org.cometd.JSON.toJSON(n.messages.slice(e,e+f));var t=n.url.length+encodeURI(q).length;if(t>b){if(f===1){var p="Bayeux message too big ("+t+" bytes, max is "+b+") for transport "+this.getType();this.setTimeout(function(){r.transportFailure(n,k,"error",p)},0);return}--f;continue}h.push(f);e+=f;f=n.messages.length-e}var m=n;if(h.length>1){var g=0;var j=h[0];this._debug("Transport",this.getType(),"split",n.messages.length,"messages into",h.join(" + "));m=this._mixin(false,{},n);m.messages=n.messages.slice(g,j);m.onSuccess=n.onSuccess;m.onFailure=n.onFailure;for(var l=1;l<h.length;++l){var o=this._mixin(false,{},n);g=j;j+=h[l];o.messages=n.messages.slice(g,j);o.onSuccess=n.onSuccess;o.onFailure=n.onFailure;this.send(o,k.metaConnect)}}this._debug("Transport",this.getType(),"sending request",k.id,"envelope",m);try{var s=true;this.jsonpSend({transport:this,url:m.url,sync:m.sync,headers:this.getConfiguration().requestHeaders,body:org.cometd.JSON.toJSON(m.messages),onSuccess:function(u){var v=false;try{var w=r.convertToMessages(u);if(w.length===0){r.transportFailure(m,k,"no response")}else{v=true;r.transportSuccess(m,k,w)}}catch(i){r._debug(i);if(!v){r.transportFailure(m,k,"bad response",i)}}},onError:function(u,i){if(s){r.setTimeout(function(){r.transportFailure(m,k,u,i)},0)}else{r.transportFailure(m,k,u,i)}}});s=false}catch(d){this.setTimeout(function(){r.transportFailure(m,k,"error",d)},0)}};return a};org.cometd.WebSocketTransport=function(){var l=new org.cometd.Transport();var h=org.cometd.Transport.derive(l);var a;var e=true;var n=false;var i={};var c={};var f=null;var k=false;var g=false;var j;function d(){var r=a.getURL().replace(/^http/,"ws");this._debug("Transport",this.getType(),"connecting to URL",r);var p=this;var o=null;var s=a.getConfiguration().connectTimeout;if(s>0){o=this.setTimeout(function(){o=null;if(!k){p._debug("Transport",p.getType(),"timed out while connecting to URL",r,":",s,"ms");p.onClose(1002,"Connect Timeout")}},s)}var q=new org.cometd.WebSocket(r);q.onopen=function(){p._debug("WebSocket opened",q);if(o){p.clearTimeout(o);o=null}if(q!==f){p._debug("Ignoring open event, WebSocket",f);return}p.onOpen()};q.onclose=function(u){var t=u?u.code:1000;var v=u?u.reason:undefined;p._debug("WebSocket closed",t,"/",v,q);if(o){p.clearTimeout(o);o=null}if(q!==f){p._debug("Ignoring close event, WebSocket",f);return}p.onClose(t,v)};q.onerror=function(){q.onclose({code:1002})};q.onmessage=function(t){p._debug("WebSocket message",t,q);if(q!==f){p._debug("Ignoring message event, WebSocket",f);return}p.onMessage(t)};f=q;this._debug("Transport",this.getType(),"configured callbacks on",q)}function m(s,p){var v=org.cometd.JSON.toJSON(s.messages);f.send(v);this._debug("Transport",this.getType(),"sent",s,"metaConnect =",p);var o=this.getConfiguration().maxNetworkDelay;var r=o;if(p){r+=this.getAdvice().timeout;g=true}var t=[];for(var q=0;q<s.messages.length;++q){var w=s.messages[q];if(w.id){t.push(w.id);var u=this;c[w.id]=this.setTimeout(function(){if(f){f.close(1000,"Timeout")}},r)}}this._debug("Transport",this.getType(),"waiting at most",r,"ms for messages",t,"maxNetworkDelay",o,", timeouts:",c)}function b(p,q){try{if(f===null){d.call(this)}else{if(k){m.call(this,p,q)}}}catch(o){this.setTimeout(function(){p.onFailure(f,p.messages,"error",o)},0)}}h.onOpen=function(){this._debug("Transport",this.getType(),"opened",f);k=true;n=true;this._debug("Sending pending messages",i);for(var p in i){var o=i[p];var q=o[0];var r=o[1];j=q.onSuccess;m.call(this,q,r)}};h.onMessage=function(u){this._debug("Transport",this.getType(),"received websocket message",u,f);var B=false;var s=this.convertToMessages(u.data);var z=[];for(var t=0;t<s.length;++t){var C=s[t];if(/^\/meta\//.test(C.channel)||C.data===undefined){if(C.id){z.push(C.id);var y=c[C.id];if(y){this.clearTimeout(y);delete c[C.id];this._debug("Transport",this.getType(),"removed timeout for message",C.id,", timeouts",c)}}}if("/meta/connect"===C.channel){g=false}if("/meta/disconnect"===C.channel&&!g){B=true}}var w=false;for(var r=0;r<z.length;++r){var p=z[r];for(var A in i){var o=A.split(",");var v=org.cometd.Utils.inArray(p,o);if(v>=0){w=true;o.splice(v,1);var x=i[A][0];var q=i[A][1];delete i[A];if(o.length>0){i[o.join(",")]=[x,q]}break}}}if(w){this._debug("Transport",this.getType(),"removed envelope, envelopes",i)}j.call(this,s);if(B){f.close(1000,"Disconnect")}};h.onClose=function(p,r){this._debug("Transport",this.getType(),"closed",p,r,f);e=n;for(var t in c){this.clearTimeout(c[t])}c={};for(var o in i){var q=i[o][0];var s=i[o][1];if(s){g=false}q.onFailure(f,q.messages,"closed "+p+"/"+r)}i={};if(f!==null&&k){f.close(1000,"Close")}k=false;f=null};h.registered=function(p,o){l.registered(p,o);a=o};h.accept=function(o,q,p){return e&&!!org.cometd.WebSocket&&a.websocketEnabled!==false};h.send=function(r,s){this._debug("Transport",this.getType(),"sending",r,"metaConnect =",s);var q=[];for(var o=0;o<r.messages.length;++o){var p=r.messages[o];if(p.id){q.push(p.id)}}i[q.join(",")]=[r,s];this._debug("Transport",this.getType(),"stored envelope, envelopes",i);b.call(this,r,s)};h.reset=function(){l.reset();if(f!==null&&k){f.close(1000,"Reset")}e=true;n=false;c={};i={};f=null;k=false;j=null};return h};org.cometd.Cometd=function(R){var aq=this;var ac=R||"default";var j=false;var Z=new org.cometd.TransportRegistry();var u;var E="disconnected";var ag=0;var X=null;var U=0;var ab=[];var p=false;var b={};var Q=0;var V=null;var N=[];var I={};var f;var D=false;var ap=false;var aa={connectTimeout:0,maxConnections:2,backoffIncrement:1000,maxBackoff:60000,logLevel:"info",reverseIncomingExtensions:true,maxNetworkDelay:10000,requestHeaders:{},appendMessageTypeToURL:true,autoBatch:false,advice:{timeout:60000,interval:0,reconnect:"retry"}};this._mixin=function(aE,aC,aF){var aG=aC||{};for(var aA=2;aA<arguments.length;++aA){var az=arguments[aA];if(az===undefined||az===null){continue}for(var aB in az){var ay=az[aB];var aD=aG[aB];if(ay===aC){continue}if(ay===undefined){continue}if(aE&&typeof ay==="object"&&ay!==null){if(ay instanceof Array){aG[aB]=this._mixin(aE,aD instanceof Array?aD:[],ay)}else{var ax=typeof aD==="object"&&!(aD instanceof Array)?aD:{};aG[aB]=this._mixin(aE,ax,ay)}}else{aG[aB]=ay}}}return aG};function A(ax){return org.cometd.Utils.isString(ax)}function w(ax){if(ax===undefined||ax===null){return false}return typeof ax==="function"}function J(az,ay){if(window.console){var ax=window.console[az];if(w(ax)){ax.apply(window.console,ay)}}}this._warn=function(){J("warn",arguments)};this._info=function(){if(aa.logLevel!=="warn"){J("info",arguments)}};this._debug=function(){if(aa.logLevel==="debug"){J("debug",arguments)}};this._isCrossDomain=function(ax){return ax&&ax!==window.location.host};function au(aD){aq._debug("Configuring cometd object with",aD);if(A(aD)){aD={url:aD}}if(!aD){aD={}}aa=aq._mixin(false,aa,aD);if(!aa.url){throw"Missing required configuration parameter 'url' specifying the Bayeux server URL"}var aC=/(^https?:\/\/)?(((\[[^\]]+\])|([^:\/\?#]+))(:(\d+))?)?([^\?#]*)(.*)?/.exec(aa.url);var aB=aC[2];var ay=aC[8];var aA=aC[9];j=aq._isCrossDomain(aB);if(aa.appendMessageTypeToURL){if(aA!==undefined&&aA.length>0){aq._info("Appending message type to URI "+ay+aA+" is not supported, disabling 'appendMessageTypeToURL' configuration");aa.appendMessageTypeToURL=false}else{var az=ay.split("/");var ax=az.length-1;if(ay.match(/\/$/)){ax-=1}if(az[ax].indexOf(".")>=0){aq._info("Appending message type to URI "+ay+" is not supported, disabling 'appendMessageTypeToURL' configuration");aa.appendMessageTypeToURL=false}}}}function t(){for(var az in b){var aA=b[az];for(var ax=0;ax<aA.length;++ax){var ay=aA[ax];if(ay&&!ay.listener){delete aA[ax];aq._debug("Removed subscription",ay,"for channel",az)}}}}function av(ax){if(E!==ax){aq._debug("Status",E,"->",ax);E=ax}}function ai(){return E==="disconnecting"||E==="disconnected"}function d(){return ++ag}function an(aB,aE,az,aC,ay){try{return aE.call(aB,aC)}catch(ax){aq._debug("Exception during execution of extension",az,ax);var aA=aq.onExtensionException;if(w(aA)){aq._debug("Invoking extension exception callback",az,ax);try{aA.call(aq,ax,az,ay,aC)}catch(aD){aq._info("Exception during execution of exception callback in extension",az,aD)}}return aC}}function z(aA){for(var az=0;az<N.length;++az){if(aA===undefined||aA===null){break}var ay=aa.reverseIncomingExtensions?N.length-1-az:az;var aC=N[ay];var aB=aC.extension.incoming;if(w(aB)){var ax=an(aC.extension,aB,aC.name,aA,false);aA=ax===undefined?aA:ax}}return aA}function c(az){for(var ay=0;ay<N.length;++ay){if(az===undefined||az===null){break}var aB=N[ay];var aA=aB.extension.outgoing;if(w(aA)){var ax=an(aB.extension,aA,aB.name,az,true);az=ax===undefined?az:ax}}return az}function al(aB,aA){var aE=b[aB];if(aE&&aE.length>0){for(var ay=0;ay<aE.length;++ay){var az=aE[ay];if(az){try{az.callback.call(az.scope,aA)}catch(ax){aq._debug("Exception during notification",az,aA,ax);var aC=aq.onListenerException;if(w(aC)){aq._debug("Invoking listener exception callback",az,ax);try{aC.call(aq,ax,az.handle,az.listener,aA)}catch(aD){aq._info("Exception during execution of listener callback",az,aD)}}}}}}}function ao(aB,aA){al(aB,aA);var aC=aB.split("/");var az=aC.length-1;for(var ay=az;ay>0;--ay){var ax=aC.slice(0,ay).join("/")+"/*";if(ay===az){al(ax,aA)}ax+="*";al(ax,aA)}}function s(){if(V!==null){org.cometd.Utils.clearTimeout(V)}V=null}function v(ax){s();var ay=I.interval+Q;aq._debug("Function scheduled in",ay,"ms, interval =",I.interval,"backoff =",Q,ax);V=org.cometd.Utils.setTimeout(aq,ax,ay)}var aw;var y;function K(aD,aC,ax,aB){for(var az=0;az<aC.length;++az){var aA=aC[az];aA.id=""+d();if(X){aA.clientId=X}aA=c(aA);if(aA!==undefined&&aA!==null){aC[az]=aA}else{aC.splice(az--,1)}}if(aC.length===0){return}var ay=aa.url;if(aa.appendMessageTypeToURL){if(!ay.match(/\/$/)){ay=ay+"/"}if(aB){ay=ay+aB}}var aE={url:ay,sync:aD,messages:aC,onSuccess:function(aG){try{aw.call(aq,aG)}catch(aF){aq._debug("Exception during handling of messages",aF)}},onFailure:function(aG,aI,aJ,aH){try{y.call(aq,aG,aI,aJ,aH)}catch(aF){aq._debug("Exception during handling of failure",aF)}}};aq._debug("Send",aE);u.send(aE,ax)}function S(ax){if(U>0||p===true){ab.push(ax)}else{K(false,[ax],false)}}this.send=S;function Y(){Q=0}function ae(){if(Q<aa.maxBackoff){Q+=aa.backoffIncrement}}function M(){++U}function i(){var ax=ab;ab=[];if(ax.length>0){K(false,ax,false)}}function H(){--U;if(U<0){throw"Calls to startBatch() and endBatch() are not paired"}if(U===0&&!ai()&&!p){i()}}function q(){if(!ai()){var ax={channel:"/meta/connect",connectionType:u.getType()};if(!ap){ax.advice={timeout:0}}av("connecting");aq._debug("Connect sent",ax);K(false,[ax],true,"connect");av("connected")}}function x(){av("connecting");v(function(){q()})}function C(ax){if(ax){I=aq._mixin(false,{},aa.advice,ax);aq._debug("New advice",I)}}function m(ax){s();if(ax){u.abort()}X=null;av("disconnected");U=0;Y();if(ab.length>0){y.call(aq,undefined,ab,"error","Disconnected");ab=[]}}function r(aA){X=null;t();if(ai()){Z.reset();C(aa.advice)}else{C(aq._mixin(false,I,{reconnect:"retry"}))}U=0;p=true;f=aA;var ax="1.0";var aB=Z.findTransportTypes(ax,j,aa.url);var ay={version:ax,minimumVersion:"0.9",channel:"/meta/handshake",supportedConnectionTypes:aB,advice:{timeout:I.timeout,interval:I.interval}};var az=aq._mixin(false,{},f,ay);u=Z.negotiateTransport(aB,ax,j,aa.url);aq._debug("Initial transport is",u.getType());av("handshaking");aq._debug("Handshake sent",az);K(false,[az],false,"handshake")}function ar(){av("handshaking");p=true;v(function(){r(f)})}function o(ay){ao("/meta/handshake",ay);ao("/meta/unsuccessful",ay);var ax=!ai()&&I.reconnect!=="none";if(ax){ae();ar()}else{m(false)}}function O(ay){if(ay.successful){X=ay.clientId;var ax=Z.negotiateTransport(ay.supportedConnectionTypes,ay.version,j,aa.url);if(ax===null){throw"Could not negotiate transport with server; client "+Z.findTransportTypes(ay.version,j,aa.url)+", server "+ay.supportedConnectionTypes}else{if(u!==ax){aq._debug("Transport",u,"->",ax);u=ax}}p=false;i();ay.reestablish=D;D=true;ao("/meta/handshake",ay);var az=ai()?"none":I.reconnect;switch(az){case"retry":Y();x();break;case"none":m(false);break;default:throw"Unrecognized advice action "+az}}else{o(ay)}}function B(ay,ax){o({successful:false,failure:true,channel:"/meta/handshake",request:ax,xhr:ay,advice:{reconnect:"retry",interval:Q}})}function T(ax){ao("/meta/connect",ax);ao("/meta/unsuccessful",ax);var ay=ai()?"none":I.reconnect;switch(ay){case"retry":x();ae();break;case"handshake":Z.reset();Y();ar();break;case"none":m(false);break;default:throw"Unrecognized advice action"+ay}}function aj(ax){ap=ax.successful;if(ap){ao("/meta/connect",ax);var ay=ai()?"none":I.reconnect;switch(ay){case"retry":Y();x();break;case"none":m(false);break;default:throw"Unrecognized advice action "+ay}}else{T(ax)}}function L(ay,ax){ap=false;T({successful:false,failure:true,channel:"/meta/connect",request:ax,xhr:ay,advice:{reconnect:"retry",interval:Q}})}function h(ax){m(true);ao("/meta/disconnect",ax);ao("/meta/unsuccessful",ax)}function am(ax){if(ax.successful){m(false);ao("/meta/disconnect",ax)}else{h(ax)}}function k(ay,ax){h({successful:false,failure:true,channel:"/meta/disconnect",request:ax,xhr:ay,advice:{reconnect:"none",interval:0}})}function ad(ax){ao("/meta/subscribe",ax);ao("/meta/unsuccessful",ax)}function at(ax){if(ax.successful){ao("/meta/subscribe",ax)}else{ad(ax)}}function n(ay,ax){ad({successful:false,failure:true,channel:"/meta/subscribe",request:ax,xhr:ay,advice:{reconnect:"none",interval:0}})}function W(ax){ao("/meta/unsubscribe",ax);ao("/meta/unsuccessful",ax)}function e(ax){if(ax.successful){ao("/meta/unsubscribe",ax)}else{W(ax)}}function l(ay,ax){W({successful:false,failure:true,channel:"/meta/unsubscribe",request:ax,xhr:ay,advice:{reconnect:"none",interval:0}})}function a(ax){ao("/meta/publish",ax);ao("/meta/unsuccessful",ax)}function G(ax){if(ax.successful===undefined){if(ax.data){ao(ax.channel,ax)}else{aq._debug("Unknown message",ax)}}else{if(ax.successful){ao("/meta/publish",ax)}else{a(ax)}}}function P(ay,ax){a({successful:false,failure:true,channel:ax.channel,request:ax,xhr:ay,advice:{reconnect:"none",interval:0}})}function ah(ay){ay=z(ay);if(ay===undefined||ay===null){return}C(ay.advice);var ax=ay.channel;switch(ax){case"/meta/handshake":O(ay);break;case"/meta/connect":aj(ay);break;case"/meta/disconnect":am(ay);break;case"/meta/subscribe":at(ay);break;case"/meta/unsubscribe":e(ay);break;default:G(ay);break}}this.receive=ah;aw=function(az){aq._debug("Received",az);for(var ax=0;ax<az.length;++ax){var ay=az[ax];ah(ay)}};y=function(ax,aC,aD,az){aq._debug("handleFailure",ax,aC,aD,az);for(var ay=0;ay<aC.length;++ay){var aB=aC[ay];var aA=aB.channel;switch(aA){case"/meta/handshake":B(ax,aB);break;case"/meta/connect":L(ax,aB);break;case"/meta/disconnect":k(ax,aB);break;case"/meta/subscribe":n(ax,aB);break;case"/meta/unsubscribe":l(ax,aB);break;default:P(ax,aB);break}}};function g(ay){var az=b[ay];if(az){for(var ax=0;ax<az.length;++ax){if(az[ax]){return true}}}return false}function F(ay,az){var ax={scope:ay,method:az};if(w(ay)){ax.scope=undefined;ax.method=ay}else{if(A(az)){if(!ay){throw"Invalid scope "+ay}ax.method=ay[az];if(!w(ax.method)){throw"Invalid callback "+az+" for scope "+ay}}else{if(!w(az)){throw"Invalid callback "+az}}}return ax}function ak(aC,aA,aE,az){var ay=F(aA,aE);aq._debug("Adding listener on",aC,"with scope",ay.scope,"and callback",ay.method);var aB={channel:aC,scope:ay.scope,callback:ay.method,listener:az};var aD=b[aC];if(!aD){aD=[];b[aC]=aD}var ax=aD.push(aB)-1;aB.id=ax;aB.handle=[aC,ax];aq._debug("Added listener",aB,"for channel",aC,"having id =",ax);return aB.handle}function af(ax){var ay=b[ax[0]];if(ay){delete ay[ax[1]];aq._debug("Removed listener",ax)}}this.registerTransport=function(az,aA,ay){var ax=Z.add(az,aA,ay);if(ax){this._debug("Registered transport",az);if(w(aA.registered)){aA.registered(az,this)}}return ax};this.getTransportTypes=function(){return Z.getTransportTypes()};this.unregisterTransport=function(ax){var ay=Z.remove(ax);if(ay!==null){this._debug("Unregistered transport",ax);if(w(ay.unregistered)){ay.unregistered()}}return ay};this.unregisterTransports=function(){Z.clear()};this.findTransport=function(ax){return Z.find(ax)};this.configure=function(ax){au.call(this,ax)};this.init=function(ay,ax){this.configure(ay);this.handshake(ax)};this.handshake=function(ax){av("disconnected");D=false;r(ax)};this.disconnect=function(az,aA){if(ai()){return}if(aA===undefined){if(typeof az!=="boolean"){aA=az;az=false}}var ax={channel:"/meta/disconnect"};var ay=this._mixin(false,{},aA,ax);av("disconnecting");K(az===true,[ay],false,"disconnect")};this.startBatch=function(){M()};this.endBatch=function(){H()};this.batch=function(az,aA){var ay=F(az,aA);this.startBatch();try{ay.method.call(ay.scope);this.endBatch()}catch(ax){this._debug("Exception during execution of batch",ax);this.endBatch();throw ax}};this.addListener=function(ay,ax,az){if(arguments.length<2){throw"Illegal arguments number: required 2, got "+arguments.length}if(!A(ay)){throw"Illegal argument type: channel must be a string"}return ak(ay,ax,az,true)};this.removeListener=function(ax){if(!org.cometd.Utils.isArray(ax)){throw"Invalid argument: expected subscription, not "+ax}af(ax)};this.clearListeners=function(){b={}};this.subscribe=function(aB,ay,aE,aD){if(arguments.length<2){throw"Illegal arguments number: required 2, got "+arguments.length}if(!A(aB)){throw"Illegal argument type: channel must be a string"}if(ai()){throw"Illegal state: already disconnected"}if(w(ay)){aD=aE;aE=ay;ay=undefined}var aC=!g(aB);var aA=ak(aB,ay,aE,false);if(aC){var ax={channel:"/meta/subscribe",subscription:aB};var az=this._mixin(false,{},aD,ax);S(az)}return aA};this.unsubscribe=function(aB,ay){if(arguments.length<1){throw"Illegal arguments number: required 1, got "+arguments.length}if(ai()){throw"Illegal state: already disconnected"}this.removeListener(aB);var aA=aB[0];if(!g(aA)){var ax={channel:"/meta/unsubscribe",subscription:aA};var az=this._mixin(false,{},ay,ax);S(az)}};this.clearSubscriptions=function(){t()};this.publish=function(aA,az,aB){if(arguments.length<1){throw"Illegal arguments number: required 1, got "+arguments.length}if(!A(aA)){throw"Illegal argument type: channel must be a string"}if(ai()){throw"Illegal state: already disconnected"}var ax={channel:aA,data:az};var ay=this._mixin(false,{},aB,ax);S(ay)};this.getStatus=function(){return E};this.isDisconnected=ai;this.setBackoffIncrement=function(ax){aa.backoffIncrement=ax};this.getBackoffIncrement=function(){return aa.backoffIncrement};this.getBackoffPeriod=function(){return Q};this.setLogLevel=function(ax){aa.logLevel=ax};this.registerExtension=function(ax,aB){if(arguments.length<2){throw"Illegal arguments number: required 2, got "+arguments.length}if(!A(ax)){throw"Illegal argument type: extension name must be a string"}var az=false;for(var ay=0;ay<N.length;++ay){var aA=N[ay];if(aA.name===ax){az=true;break}}if(!az){N.push({name:ax,extension:aB});this._debug("Registered extension",ax);if(w(aB.registered)){aB.registered(ax,this)}return true}else{this._info("Could not register extension with name",ax,"since another extension with the same name already exists");return false}};this.unregisterExtension=function(ay){if(!A(ay)){throw"Illegal argument type: extension name must be a string"}var ax=false;for(var az=0;az<N.length;++az){var aB=N[az];if(aB.name===ay){N.splice(az,1);ax=true;this._debug("Unregistered extension",ay);var aA=aB.extension;if(w(aA.unregistered)){aA.unregistered()}break}}return ax};this.getExtension=function(ax){for(var ay=0;ay<N.length;++ay){var az=N[ay];if(az.name===ax){return az.extension}}return null};this.getName=function(){return ac};this.getClientId=function(){return X};this.getURL=function(){return aa.url};this.getTransport=function(){return u};this.getConfiguration=function(){return this._mixin(true,{},aa)};this.getAdvice=function(){return this._mixin(true,{},I)};org.cometd.WebSocket=window.WebSocket;if(!org.cometd.WebSocket){org.cometd.WebSocket=window.MozWebSocket}};
if(typeof dojo!="undefined"){dojo.provide("org.cometd.AckExtension")}org.cometd.AckExtension=function(){var b;var a=false;var c=-1;function d(f,e){b._debug(f,e)}this.registered=function(f,e){b=e;d("AckExtension: executing registration callback")};this.unregistered=function(){d("AckExtension: executing unregistration callback");b=null};this.incoming=function(g){var f=g.channel;if(f=="/meta/handshake"){a=g.ext&&g.ext.ack;d("AckExtension: server supports acks",a)}else{if(a&&f=="/meta/connect"&&g.successful){var e=g.ext;if(e&&typeof e.ack==="number"){c=e.ack;d("AckExtension: server sent ack id",c)}}}return g};this.outgoing=function(f){var e=f.channel;if(e=="/meta/handshake"){if(!f.ext){f.ext={}}f.ext.ack=b&&b.ackEnabled!==false;c=-1}else{if(a&&e=="/meta/connect"){if(!f.ext){f.ext={}}f.ext.ack=c;d("AckExtension: client sending ack id",c)}}return f}};
if(typeof dojo!="undefined"){dojo.provide("org.cometd.ReloadExtension")}if(!org.cometd.COOKIE){org.cometd.COOKIE={};org.cometd.COOKIE.set=function(b,c,a){throw"Abstract"};org.cometd.COOKIE.get=function(a){throw"Abstract"}}org.cometd.ReloadExtension=function(d){var a;var e;var f=null;var j="org.cometd.reload";var i="/";var b=5;var h=false;function g(l){if(f&&f.handshakeResponse!==null){c(l);f.cookiePath=i;var m=org.cometd.JSON.toJSON(f);e("Reload extension saving cookie value",m);org.cometd.COOKIE.set(j,m,{"max-age":b,path:i,expires:new Date(new Date().getTime()+b*1000)})}}function k(l){return f.url==l.url}function c(l){if(l){if(typeof l.cookieMaxAge==="number"){b=l.cookieMaxAge}if(typeof l.cookieName==="string"){j=l.cookieName}if(typeof l.cookiePath==="string"){i=l.cookiePath}}}this.configure=c;this.registered=function(m,l){a=l;a.reload=g;e=a._debug};this.unregistered=function(){delete a.reload;a=null};this.outgoing=function(p){var o=p.channel;if(o=="/meta/handshake"){f={};f.url=a.getURL();var n=org.cometd.COOKIE.get(j);e("Reload extension found cookie value",n);if(n){try{var m=org.cometd.JSON.fromJSON(n);org.cometd.COOKIE.set(j,"",{"max-age":-1,path:m.cookiePath,expires:-1});if(m.handshakeResponse&&k(m)){e("Reload extension restoring state",m);setTimeout(function(){e("Reload extension replaying handshake response",m.handshakeResponse);f.handshakeResponse=m.handshakeResponse;f.transportType=m.transportType;f.reloading=true;var q=a._mixin(true,{},f.handshakeResponse,{ext:{reload:true}});q.supportedConnectionTypes=[f.transportType];a.receive(q);e("Reload extension replayed handshake response",q)},0);if(!h){h=true;a.startBatch()}return null}else{e("Reload extension could not restore state",m)}}catch(l){e("Reload extension error while trying to restore cookie",l)}}}else{if(o=="/meta/connect"){if(!f.transportType){f.transportType=p.connectionType;e("Reload extension tracked transport type",f.transportType)}}}return p};this.incoming=function(l){if(l.successful){switch(l.channel){case"/meta/handshake":if(!f.handshakeResponse){f.handshakeResponse=l;e("Reload extension tracked handshake response",l)}break;case"/meta/disconnect":f=null;break;case"/meta/connect":if(h){a.endBatch();h=false}break;default:break}}return l};c(d)};
if(typeof dojo!="undefined"){dojo.provide("org.cometd.TimeStampExtension")}org.cometd.TimeStampExtension=function(){this.outgoing=function(a){a.timestamp=new Date().toUTCString();return a}};
if(typeof dojo!="undefined"){dojo.provide("org.cometd.TimeSyncExtension")}org.cometd.TimeSyncExtension=function(h){var e;var g=h&&h.maxSamples||10;var d=[];var c=[];var a=0;var b=0;function f(j,i){e._debug(j,i)}this.registered=function(j,i){e=i;f("TimeSyncExtension: executing registration callback")};this.unregistered=function(){f("TimeSyncExtension: executing unregistration callback");e=null;d=[];c=[]};this.incoming=function(s){var o=s.channel;if(o&&o.indexOf("/meta/")===0){if(s.ext&&s.ext.timesync){var p=s.ext.timesync;f("TimeSyncExtension: server sent timesync",p);var j=new Date().getTime();var m=(j-p.tc-p.p)/2;var k=p.ts-p.tc-m;d.push(m);c.push(k);if(c.length>g){c.shift();d.shift()}var q=c.length;var r=0;var l=0;for(var n=0;n<q;++n){r+=d[n];l+=c[n]}a=parseInt((r/q).toFixed());b=parseInt((l/q).toFixed());f("TimeSyncExtension: network lag",a,"ms, time offset with server",b,"ms",a,b)}}return s};this.outgoing=function(j){var i=j.channel;if(i&&i.indexOf("/meta/")===0){if(!j.ext){j.ext={}}j.ext.timesync={tc:new Date().getTime(),l:a,o:b};f("TimeSyncExtension: client sending timesync",org.cometd.JSON.toJSON(j.ext.timesync))}return j};this.getTimeOffset=function(){return b};this.getTimeOffsetSamples=function(){return c};this.getNetworkLag=function(){return a};this.getServerTime=function(){return new Date().getTime()+b};this.getServerDate=function(){return new Date(this.getServerTime())};this.setTimeout=function(m,l){var k=(l instanceof Date)?l.getTime():(0+l);var i=k-b;var j=i-new Date().getTime();if(j<=0){j=1}return org.cometd.Utils.setTimeout(e,m,j)}};
