...
chrome.browserAction.onClicked.addListener(captureDesktopForSharing);

function captureDesktopForSharing() {
...
function setupRTCMultiConnection(stream) {
    ...
    websocket.onopen = function() {
        ...
        var url = "data:text/html;charset=UTF-8,<h1>화면공유를 하시려면 아래 URL를 복사해서 전달해주세요:</h1>";
        url += "<input type='text' value='" + resultingURL + "' style='width:100%;font-size:1.2em;'>";
        url += "<br><font color='red'>Chrome, Firefox에서만 가능합니다. (IE에서는 사용불가)";
        chrome.windows.create({
